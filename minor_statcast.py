...
def game_pks_from_date(start_dt: str, end_dt: str, sport_id: str = "11", team_id: str = None) -> List[str]:
    """ 
    ...
    """
    content = json.loads(requests.get("https://statsapi.mlb.com/api/v1/schedule", params={"sportId": sport_id, "startDate": start_dt, "endDate": end_dt, "hydrate": "team", "teamId": team_id}).content)
    dates = content['dates']
...
def pitchdata_from_pks(pks: list[str], statcast_only = True, max_net_threads: int =  10) -> pd.DataFrame:
    """ 
    ...
    with tqdm(total=len(futures), desc="Gathering Data", unit="Game") as pbar:
        for completed_future in as_completed(futures):
            ...
            if velo_data.size > 0 or not statcast_only:
                ...
                if velo_data.size > 0:
                    PA_data = PA_data.merge(velo_data, on="play_id", how="left")
